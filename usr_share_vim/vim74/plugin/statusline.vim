...
fun! s:SetFullStatusline() "{{{
  ...
  setlocal statusline+=%h%#StatuslineFlag#%F%m%r%h%w                 " flags
  "setlocal statusline+=%#StatuslinePath#\ [%-0.20{StatusLineGetPath()}]\  " path
  ...
  setlocal statusline+=\ %#StatuslineTime#%{strftime(\"%y/%m/%d\ -\ %H:%M:%S\")}\ \ " current time
