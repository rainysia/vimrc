...
161. info -f linux-command -n 'keyword'
...
171, smartctl to check drive
smartctl -a -s on /dev/sda

172. cat json.txt | python -mjson.tool 用python来格式化输出json字符串

173. shell 追加path环境变量
