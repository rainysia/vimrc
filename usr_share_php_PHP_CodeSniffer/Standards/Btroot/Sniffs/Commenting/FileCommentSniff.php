...
class Btroot_Sniffs_Commenting_FileCommentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    /**
     * Author tag must be 'Xxx Yyyy<yyyx@btroot.org>'.
     *
     * @param int $errorPos The line number where the error occurs.
     *
     * @return void
    ...
    protected function processAuthors($errorPos)
    {
        ...
        if (empty($authors) === false) {
            ...
            } else if ( ! preg_match('%^.*\<[a-z0-9]+\@btroot\.org\>$%', $content) ) {
                $error = 'Expected "Xxx Yyy<yyyx@btroot.org>" for author tag';
    ...
    protected function processCopyrights($errorPos)
    {
        ...
        if ($copyright !== null) {
            ...
            } else if (preg_match('/^([0-9]{4})(-[0-9]{4})? (Btroot)?\.[a-zA-z]+\.(Team)?$/', $content) === 0) {
                $error = 'Expected "xxxx-xxxx Btroot.xxx.Team" for copyright declaration';
            //} else if (preg_match('/^([0-9]{4})(-[0-9]{4})? (0-9a-zA-Z_-)?\.[a-zA-z]+\.(Team)?$/', $content) === 0) {
             //   $error = 'Expected "xxxx-xxxx xxx.xxx.Team" for copyright declaration';
    ...
    protected function processLicense($errorPos)
    {
        ...
        if ($license !== null) {
            ...
            if (empty($url) === true && empty($content) === true) {
                ...
                } else if (preg_match('/^([a-zA-Z]{1,5})?(:\/\/)?[\s\S]*$/', $url) === 0) {
                    $error = 'Expected "xxx://xxxxxx..." for license URL';
                    $this->currentFile->addError($error, $errorPos, 'IncorrectLicenseURL');
                }

                // Check for license name.
                if (empty($content) === true) {
                    $error = 'License name missing for @license tag in file comment at the end of line';
                    $this->currentFile->addError($error, $errorPos, 'MissingLicenseName');
                //} else if ($content !== 'GPLv2') {
                    //$error = 'Expected "GPLv2" for license name';
                } else if (preg_match('/[\s\S]*$/', $content) === 0) {
                    $error = 'Expected "xxxx" for license name at the end of line';
